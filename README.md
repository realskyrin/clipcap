...
1. 在“设置 > 通用”中开启“自动打开系统截图”
2. 选择自动配置，或在系统截图工具中把存储位置手动设为 `~/Pictures/ClipCap Screenshots`
3. 使用 macOS 系统截图快捷键，截图载入编辑器后会立即从专用文件夹删除，编辑完成时仍按现有设置写入 ClipCap 历史记录
4. 为了最快衔接，请在系统截图的选项中关闭“显示浮动缩略图”

不希望更改系统截图位置时，可以按 `Control + Shift + Command + 4` 把截图复制到剪贴板，再使用“编辑剪贴板图片”。也可以把图片文件拖到应用、使用“打开图片”、从 Finder 里选择“Open With clipcap”，或把图片复制到剪贴板后交给 clipcap 编辑

## 功能

- 编辑剪贴板图片和本地图片文件
- 自动监听专用系统截图文件夹，排队打开连续截图，并在载入编辑器后删除源文件
- 箭头、矩形、圆形、线条、画笔、荧光笔、马赛克、文字、编号、贴图、二维码识别
- OCR、截图翻译、词典模式和翻译服务配置
- 图片上传到自定义图床，并复制链接或 Markdown
- 保存到本地文件夹，保留并重新复制历史项目；悬停项目可点击右下角星标收藏，收藏筛选支持批量收藏，收藏项不受自动清理、关闭缓存开关、“删除全部历史”或删除选中项影响，关闭对应缓存后仍可在历史面板查看
- 从菜单栏保持 Mac 唤醒，支持无限期、预设时长和指定结束时间，无需额外权限
- 菜单栏常驻，无 Dock 图标

## 隐私边界

clipcap 的输入来自系统剪贴板、打开面板、文件拖拽、Open With、用户明确选择的图片文件，以及用户主动开启的专用系统截图文件夹。应用不会请求或复用旧应用的 TCC 权限，不会使用全局热键触发屏幕捕获，也不会通过 Finder Automation 读取选中文件
