...
/// Locates and reads the most recent clipcap diagnostic log. This includes the
...
enum CrashLogReader {
    ...
    /// name matches clipcap's CFBundleExecutable.
    private static let processName = "clipcap"

    /// Returns the newest app diagnostic log or macOS crash report belonging
    /// to clipcap, or nil if no log exists. This only stats the files — it does
    ...
    /// crash report belonging to clipcap, or nil if the app has never crashed.
    ...
    /// Deletes clipcap's persisted diagnostic log and any clipcap-owned macOS
