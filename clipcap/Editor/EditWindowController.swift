...
private let keyRightArrow: Int = 124
...
class EditWindowController {
    ...
    // Legacy capture state is kept false in clipcap. The editor works from
    // explicit images supplied by clipboard, file, drag, Open With, or history.
    ...
    private func updateCaptureActionAvailability() {
        toolbars.forEach {
            $0.setScrollCaptureEnabled(false)
        }
    }
    ...
    // MARK: - Unavailable Capture Paths

    private func toggleScrollCapture() {
        ToastWindow.show(message: L10n.openImageNoImage, on: screen)
    }

    private func stopScrollCapture(reason: String = "unknown") {
        logScrollCapture("unavailable-stop", metadata: ["reason": reason])
    ...
    private func logScrollCapture(_ event: String, metadata: [String: Any] = [:]) {
        ...
        DiagnosticLog.log("image-crop", event, metadata: fields)
    }
...
final class HUDSlider: NSControl {
    ...
    override func keyDown(with event: NSEvent) {
        ...
        switch Int(event.keyCode) {
        case keyLeftArrow, keyDownArrow:
            onEditingBegan?()
            setValue(value - step, notify: true)
            onEditingEnded?()
        case keyRightArrow, keyUpArrow:
            onEditingBegan?()
            setValue(value + step, notify: true)
            onEditingEnded?()
        default:
