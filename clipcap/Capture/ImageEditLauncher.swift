...
enum ImageEditLauncher {
    ...
    /// show a clear input error. The source file is copied
    ...
    /// nil for an empty or zero-size image so the caller can show a prompt.
    static func launch(
    ...
    private static func tempDir() -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent("clipcap-edit", isDirectory: true)
    }
    ...
    private static func copyToTemp(_ source: URL) -> URL? {
        ...
        do {
            ...
            NSLog("clipcap: failed to copy image to temp: \(error)")
            return nil
        }
    }
